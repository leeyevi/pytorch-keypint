...
def main(args):
    ...
    checkpoint_file = os.path.join(args.checkpoint, 'epoch500checkpoint.pth.tar')#args.test+'.pth.tar')
    ...
    for i, (inputs, meta) in tqdm(enumerate(test_loader)):
        ...
        #     break
        ctest2 = time.clock()
        print('ctest1:%s s' % (ctest2 - ctest1))
        with torch.no_grad():
            ...
            for b in range(inputs.size(0)):
                ...
                if len(single_result) != 0:
                    ...
                    for hh in range(0, len(single_result), 3):
                        cv2.circle(img, (int(single_result[hh]), int(single_result[hh+1])), radius=1, color=(255, 0, 255),thickness=-1)
                        cv2.putText(img,str(ig),(int(single_result[hh])+2, int(single_result[hh+1])+2),cv2.FONT_HERSHEY_COMPLEX,0.3,(0,0,255),1)
                        ig = ig+1
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--checkpoint', default='checkpoint', type=str, metavar='PATH',
                        help='path to load checkpoint (default: checkpoint)')#checkpoint910 base
