...
class MscocoMulti(data.Dataset):
    def __init__(self, cfg, img, train=True):
    ...
    def __getitem__(self, index):
        ...
        #gt_bbox = []
        image = self.image #scipy.misc.imread(img_path, mode='RGB')lyw rgb
        gt_bbox = [0, 0, image.shape[1], image.shape[0]]
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        #image = cv2.cvtColor()图像通道转换
        #if self.is_train:
